...
public class Ball {
    ...
    public Ball(float x,float y,float size,float xMovement,float yMovement,Texture img,float movementFaktor) {
        ...
        hitCircle=new Circle(position.x,position.y,size);
        paddleheight=Gdx.graphics.getHeight()/20.0f;
    ...
    public void checkCollision(Paddle paddle0,Paddle paddle1,Paddle paddle2,Paddle paddle3){
        if(!outsideOfCatch()) {
            if (hitBox.overlaps(paddle0.hitBox)) {
                ...
                if (movement.x < 0) {
                    movement.x -= (random.nextInt(25) + 1) * movementFaktor;
                } else {
                    movement.x += (random.nextInt(25) + 1) * movementFaktor;
                }
            }
            ...
            if (hitBox.overlaps(paddle2.hitBox)) {
                ...
                if (movement.y < 0) {
                    movement.y -= (random.nextInt(25) + 1) * movementFaktor;
                } else {
                    movement.y += (random.nextInt(25) + 1) * movementFaktor;
                }
            }
    ...
    public boolean outsideOfScreen(){
        return position.x< -Gdx.graphics.getWidth()/2.0f-size||position.x> Gdx.graphics.getWidth()/2.0f
                ||position.y<-Gdx.graphics.getHeight()/2.0f-size||position.y>Gdx.graphics.getHeight()/2.0f;
    }
    public boolean outsideOfCatch(){
        return position.x< -Gdx.graphics.getWidth()/2.0f+paddleheight||position.x> Gdx.graphics.getWidth()/2.0f-size-paddleheight
                ||position.y<-Gdx.graphics.getHeight()/2.0f+paddleheight||position.y>Gdx.graphics.getHeight()/2.0f-size-paddleheight;
    }
